...
#ifndef TEMPUS_TEST_BRUSELLATOR_MODEL_IMPL_HPP
#define TEMPUS_TEST_BRUSELLATOR_MODEL_IMPL_HPP
...
BrusselatorModel<Scalar>::
BrusselatorModel(Teuchos::RCP<Teuchos::ParameterList> pList_)
{
  isInitialized_ = false;
  dim_ = 2;
  Np_ = 0; // Number of parameter vectors (1)
  np_ = 0; // Number of parameters in this vector (1)
...
getExactSolution(double t) const
{
  TEUCHOS_TEST_FOR_EXCEPTION( true, std::logic_error,
      "Error - No exact solution for Brusselator problem!\n");
  return(inArgs_);
}
...
getExactSensSolution(int j, double t) const
{
  TEUCHOS_TEST_FOR_EXCEPTION( !isInitialized_, std::logic_error,
      "Error - No exact sensitivities for Brusselator problem!\n");
  return(inArgs_);
}
  ...
  ) const
{
  ...
  if (inArgs.get_x_dot().is_null()) {
    ...
    if (!is_null(f_out)) {
      ...
      f_out_view[0] = 1.0 + x_in_view[1]*x_in_view[0]*x_in_view[0] - 4.0*x_in_view[0];
      f_out_view[1] = 3.0*x_in_view[0] - x_in_view[1]*x_in_view[0]*x_in_view[0];
    ...
    if (!is_null(W_out)) {
      ...
      // d(f0)/d(x0_n)
      matrix_view(0,0) = beta*(2.0*x_in_view[0]*x_in_view[1] - 4.0*x_in_view[0]*x_in_view[0]);
      // d(f0)/d(x1_n)
      matrix_view(0,1) = beta*x_in_view[0]*x_in_view[0];
      // d(f1)/d(x0_n)
      matrix_view(1,0) = beta*(3.0 - 2.0*x_in_view[0]*x_in_view[1]);
      // d(f1)/d(x1_n)
      matrix_view(1,1) = -beta*x_in_view[0]*x_in_view[0];
    ...
    if (!is_null(f_out)) {
      ...
      f_out_view[0] = x_dot_in_view[0] - (1.0 + x_in_view[1]*x_in_view[0]*x_in_view[0] - 4.0*x_in_view[0]);
      f_out_view[1] = x_dot_in_view[1] - (3.0*x_in_view[0] - x_in_view[1]*x_in_view[0]*x_in_view[0]);
    }
    if (!is_null(W_out)) {
      RCP<Thyra::MultiVectorBase<Scalar> > matrix =
        Teuchos::rcp_dynamic_cast<Thyra::MultiVectorBase<Scalar> >(W_out,true);
      Thyra::DetachedMultiVectorView<Scalar> matrix_view( *matrix );
      matrix_view(0,0) = alpha - beta*(2.0*x_in_view[0]*x_in_view[1] - 4.0*x_in_view[0]*x_in_view[0]);
      matrix_view(0,1) = -beta*x_in_view[0]*x_in_view[0];
      matrix_view(1,0) = -beta*(3.0 - 2.0*x_in_view[0]*x_in_view[1]);
      matrix_view(1,1) = alpha - beta*x_in_view[0]*x_in_view[0];
      // Note: alpha = d(xdot)/d(x_n) and beta = d(x)/d(x_n)
    }
    if (!is_null(DfDp_out)) {
      Thyra::DetachedMultiVectorView<Scalar> DfDp_out_view( *DfDp_out );
...
setParameterList(Teuchos::RCP<Teuchos::ParameterList> const& paramList)
{
  ...
  Teuchos::RCP<ParameterList> tmpPL = Teuchos::rcp(new ParameterList("BrusselatorModel"));
...
getValidParameters() const
{
  ...
  if (is_null(validPL)) {
    ...
    Teuchos::setDoubleParameter(
        "IC x0", 1.01, "Initial Condition for x0", &*pl);
    Teuchos::setDoubleParameter(
        "IC x1", 3.0, "Initial Condition for x1", &*pl);
    Teuchos::setDoubleParameter(
...
#endif // TEMPUS_TEST_BRUSELLATOR_MODEL_IMPL_HPP
