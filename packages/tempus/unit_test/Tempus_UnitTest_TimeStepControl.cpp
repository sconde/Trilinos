...
TEUCHOS_UNIT_TEST(TimeStepControl, Default_Construction)
{
  ...
  tsc->initialize();  TEUCHOS_TEST_FOR_EXCEPT(!tsc->isInitialized());
...
TEUCHOS_UNIT_TEST(TimeStepControl, Full_Construction)
{
  ...
  auto tsc = rcp(new Tempus::TimeStepControl<double>(
    "Constant", 1.0, 100.0, 0.01, 0.02, 0.05, -100,
...
TEUCHOS_UNIT_TEST(TimeStepControl, getValidParameters)
{
  ...
  Teuchos::RCP<const Teuchos::ParameterList> pl = tsc->getValidParameters();

  TEST_COMPARE          ( pl->get<std::string>("Integrator Step Type")     , ==, "Constant"      );
...
{
  auto tscDefault = rcp(new Tempus::TimeStepControl<double>());
  TEUCHOS_TEST_FOR_EXCEPT(!tscDefault->isInitialized());

  auto pl = Teuchos::rcp_const_cast<ParameterList>(tscDefault->getValidParameters());

  pl->set<std::string>("Integrator Step Type"  , "Constant");
  ...
  auto tscsPL = Teuchos::parameterList("Time Step Control Strategy");
